...
const getChannelByHostId = (request, response) => {
    ...
    pool.query('SELECT * FROM channels WHERE host_id = $1', [host_id], (error, results) => {
        if(error){
            throw error
        }
        response.status(200).json(results.rows)
...
const getChannelByJoinCode = (request, response) => {
    ...
    pool.query('SELECT * FROM channels WHERE join_code = $1', [join_code], (error, results) => {
        if(error){
            throw error
        }
        response.status(200).json(results.rows)
...
const createChannel = (request, response) => {
    ...
    pool.query('INSERT INTO channels (host_id, join_code) VALUES ($1, $2)', [host_id, join_code], (error, results) => {
        if (error) {
            throw error
        }
